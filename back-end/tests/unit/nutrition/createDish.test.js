...
describe('POST /dishes - createDish', () => {
  ...
  it('CD01: Creates dish successfully with valid data', async () => {
    ...
    expect(res.body.category[0]._id.toString()).toBe(meal._id.toString());
    expect(res.body.school_id.toString()).toBe(school._id.toString());
  });

  it('CD02: Returns populated category array', async () => {
    const school = await createTestSchool();
    const staff = await createUserWithRole(school._id, 'nutrition_staff');
    ...
    expect(res.body.category[0].meal).toBe('Chiều');
  });
  ...
  it('CD03: Missing dish_name returns 400', async () => {
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ description: 'desc', category: [meal._id.toString()] })
      .expect(400);
  ...
  it('CD04: Missing description returns 400', async () => {
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ dish_name: 'Món', category: [meal._id.toString()] })
      .expect(400);

    expect(res.body.error).toContain('description');
  });

  it('CD05: Missing category returns 400', async () => {
    const school = await createTestSchool();
    const staff = await createUserWithRole(school._id, 'nutrition_staff');
    const app = buildAppWithUser({ id: staff._id.toString(), role: 'nutrition_staff', school_id: school._id.toString() });
  ...
  it('CD06: Invalid category ID returns 400', async () => {
    const school = await createTestSchool();
    const staff = await createUserWithRole(school._id, 'nutrition_staff');
    const app = buildAppWithUser({ id: staff._id.toString(), role: 'nutrition_staff', school_id: school._id.toString() });
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ dish_name: 'Món', description: 'desc', category: ['507f1f77bcf86cd799439011'] })
      .expect(400);

    expect(res.body.error).toContain('category không hợp lệ');
  });

  it('CD07: Empty category array returns 400', async () => {
    const school = await createTestSchool();
    const staff = await createUserWithRole(school._id, 'nutrition_staff');
    const app = buildAppWithUser({ id: staff._id.toString(), role: 'nutrition_staff', school_id: school._id.toString() });
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ dish_name: 'Món', description: 'desc', category: [] })
      .expect(400);
  ...
  it('CD08: User without school_id returns 403', async () => {
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ dish_name: 'Món', description: 'desc', category: [meal._id.toString()] })
      .expect(403);
  ...
  it('CD09: Creates dish with very long name and description', async () => {
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ dish_name: longName, description: longDesc, category: [meal._id.toString()] })
      .expect(201);
  ...
  it('CD10: Accepts dish_name with special characters', async () => {
    ...
    const res = await request(app)
      .post('/dishes')
      .send({ dish_name: name, description: 'desc', category: [meal._id.toString()] })
      .expect(201);
